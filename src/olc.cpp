...
do_mset (CHAR_DATA * ch, char *argument, int cmd)
{
      ...
      else if (!str_cmp (subcmd, "venom"))
  {
    ...
    if (!*buf)
      {
        send_to_char ("What venom type did you wish to set?\n", ch);
        return;
      }
    if ((ind = lookup_value (buf, REG_MAGIC_SPELLS)) == -1)
      {
        send_to_char ("I couldn't find that affect in the database.\n",
          ch);
        return;
      }
    if (ind > MAGIC_LAST_POISON || ind < MAGIC_FIRST_POISON)
      {
    ...
    if (!*buf)
      {
        send_to_char
    ("You'll need to specify a variable duration, e.g. 4d5.\n",
     ch);
        return;
      }
    if (!str_cmp (buf, "remove"))
      {
        if (edit_mob->venom)
    {
      if (edit_mob->venom->poison_type == ind)
        edit_mob->venom = edit_mob->venom->next;
      else
        for (poison = edit_mob->venom; poison;
       poison = poison->next)
          {
      if (poison->next->poison_type == ind)
        poison->next = poison->next->next;
          }
    }
        send_to_char ("The specified venom type has been removed.\n",
          ch);
        return;
      }
    else
      {
        sscanf (buf, "%dd%d", &dice, &sides);
        if ((dice < 1 || sides < 1) && str_cmp (buf, "remove"))
    {
      send_to_char ("You'll need a value of at least 1d1.\n", ch);
      return;
    }
        if (!edit_mob->venom)
    {
      CREATE (edit_mob->venom, POISON_DATA, 1);
      edit_mob->venom->poison_type = ind;
      edit_mob->venom->duration_die_1 = dice;
      edit_mob->venom->duration_die_2 = sides;
      edit_mob->venom->effect_die_1 = 0;
      edit_mob->venom->effect_die_2 = 0;
      edit_mob->venom->next = NULL;
    }
        else
    for (poison = edit_mob->venom; poison; poison = poison->next)
      {
        if (!poison->next)
          {
      CREATE (poison->next, POISON_DATA, 1);
      poison->next->poison_type = ind;
      poison->next->duration_die_1 = dice;
      poison->next->duration_die_2 = sides;
      poison->effect_die_1 = 0;
      poison->effect_die_2 = 0;
      poison->next = NULL;
      break;
          }
      }
        send_to_char ("The specified venom type has been added.\n", ch);
        return;
      }
  }
    ...
    *buf = tolower (*buf);
    ...
    ("That mobile will now deal in Numenorean currency (Engrens).\n",
     ch);
      ...
      {
        send_to_char
    ("Specify Yrcish, Gondorian, or Numenorean currency.\n", ch);
        return;
      }
  }
