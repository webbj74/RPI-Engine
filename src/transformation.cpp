  ...
  if (target_type == TARGET_OBJ)
	  ...
	  && tobj->nVirtual == VNUM_CORPSE)
	{
	  if ((spell->a.spell.discipline == SKILL_WHITE_WISE
	       || spell->a.spell.discipline == SKILL_GREY_WISE)
	    ...
	    if ((spell->a.spell.discipline == SKILL_GREY_WISE
		 || spell->a.spell.discipline == SKILL_WHITE_WISE)
		&& spell->a.spell.magnitude >= MAGNITUDE_POWERFUL)
	    {
	      sprintf (buf,
      ...
      if (spell->a.spell.discipline == SKILL_GREY_WISE)
	{
      ...
      else if (spell->a.spell.discipline == SKILL_WHITE_WISE)
	{
	  for (wound = tch->wounds; wound; wound = next_wound)
	    {
	      next_wound = wound->next;
	      wound->healerskill =
		ch->skills[SKILL_WHITE_WISE] + (spell->a.spell.magnitude * 5);
      ...
      else if (spell->a.spell.discipline == SKILL_BLACK_WISE)
	{
	  ...
	  for (wound = tch->wounds; wound; wound = next_wound)
	    {
	      next_wound = wound->next;
	      if (((ch->skills[SKILL_BLACK_WISE] / 5 +
