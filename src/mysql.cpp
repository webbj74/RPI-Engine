...
init_mysql (void)
{
  ...
  (database, MYSQL_HOST, MYSQL_USERNAME, NULL, 0, 0, 0, 0)))
  /**
	 (database,
...
/********** remove comments to use MRTG tracking 
			...
			char *argument)
{
  ...
  if (board_type == 0)
	...
	return 0;
      if ((!str_cmp (name, "Bugs")
      	|| !str_cmp (name, "Typos")
	   		|| !str_cmp (name, "Ideas")
	   		|| !str_cmp (name, "Mordor-Petitions")
	   		|| !str_cmp (name, "Gondor-Petitions")
