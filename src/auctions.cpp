...
#define NO_MIN_BID      "What was the minimum bid in coppers you wished to list for this item?"
...
#define LENGTH_SANITY    "I am afraid we will only list merchandise for a period of one to thirty days."
...
#define HOW_MUCH      "How many coppers did you wish to bid on this auction?"
...
*/
  ...
  *buf2 = '\0';
  ...
  if ( type == AUCTION_BID )
  {
    /* Let's notify the seller first, if they're online. */
    ...
    /* Now, the unlucky sod who was just outbid: */
  ...
  if ( type == AUCTION_SALE )
  {
    ...
    /* And now the high bidder who lost the item: */
    if ( (tch = get_char_nomask (row[9])) )
    {
...
retrieve_expiries (CHAR_DATA *ch, CHAR_DATA *auctioneer)
{
  ...
  {
    name_to_ident (ch, buf2);
    sprintf (buf,  "%s Better luck next time perhaps - I have %d coppers here from an unsuccessful bid you placed some time ago. Here you are.", buf2, atoi(row[1]));     
    do_whisper (auctioneer, buf, 83);
...
retrieve_winnings (CHAR_DATA *ch, CHAR_DATA *auctioneer)
{
  ...
  if ( !str_cmp (row[11], GET_NAME(ch)) && atoi(row[20]) == false )
  {
    ...
    name_to_ident (ch, buf2);
    sprintf (buf,  "%s Here we are. For the successful auction of #2%s#0, you will receive %d coppers, plus a refund of your %d copper deposit, minus a House consignment "
            "fee of %d coppers, for a net total of %d coppers. Enjoy!", buf2, row[15], sold_for, atoi(row[5]), fee, net); 
...
place_bid (CHAR_DATA *ch, CHAR_DATA *auctioneer, int id, int bid, bool confirmed)
{
  ...
  *buf3 = '\0';
  ...
  {
    name_to_ident (ch, buf2);
    sprintf (buf, "%s I am sorry, but the next available bid on this auction is set at a minimum of %s coppers.", buf2, row[9]);
    do_whisper (auctioneer, buf, 83);
    if ( result )
      mysql_free_result (result);
    return;
  ...
  if ( !confirmed ) 
  {
    ...
    sprintf (buf,  "To make sure we understand one another - you wish to place a bid of %d coppers upon #2%s#0. "
  ...
  int min_bid, int buyout, int length, bool confirmed)
{
  ...
  *buf3 = '\0';
  ...
  deposit = (int) round ((obj->farthings + (obj->silver * 4)) * .08);
  
  if ( !deposit )    // Item doesn't have a "real" value we can get deposit info from ...
    deposit = (int) round (min_bid * .05);

  // Tack on an extra 1.75% deposit per day of auctioning.
  
  deposit += (int) round (deposit * (length * .0175));
  ...
  sprintf (buf3, " and a buyout of %d coppers", buyout);
  
  sprintf (buf,  "Let's see if I have this right: you wish to auction #2%s#0 for a minimum bid of %d coppers%s, "
          "and you wish it to be listed for %d days. We will require you to place a deposit of "
          "%d coppers that will be returned upon the successful sale of your auction. Is this agreeable?", 
...
/** auction place item <item> <minimum bid> (<buyout>) <length>**/    
