         ...
         AFFECTED_TYPE * craft_affect)
{
  ...
  if (get_affect (ch, MAGIC_CRAFT_DELAY)
      && (craft_affect->a.craft->subcraft->delay || craft_affect->a.craft->subcraft->faildelay) && IS_MORTAL (ch))
    {
      act
...
meets_skill_requirements (CHAR_DATA * ch, SUBCRAFT_HEAD_DATA * craft)
{
      ...
      if (phase->skill)
  {
    if (!real_skill (ch, phase->skill)
        ...
        ((int) ((phase->dice * phase->sides) * 0.750)))
      return 0;
  }
    }
 ...
 /* Check to see if attribute check succeeded.  This really is
  ...
  for (i = 0; i < MAX_ITEMS_PER_SUBCRAFT; i++)
    ...
    if (!item)
      ...
      if (      /*!item_required [i] || */
     item->phase != phase || !IS_SET (item->flags, SUBCRAFT_USED))
  continue;
...
/* Fails - We need to account for the quantity of an item */
    ...
    *color = '\0';
    ...
    if (color && IS_SET (vtoo(item->items[0])->obj_flags.extra_flags,           ITEM_VARIABLE))
        ...
        fobj_list[i] = load_colored_object (item->items[0], color);
      else
